"""
Protocol parsers for the capture engine.

Each parser takes a scapy packet and returns a ParsedPacket dataclass,
or None if the packet doesn't match that protocol. The ParsedPacket's
`data` dict holds protocol-specific fields for fingerprinting.
"""
...
def parse_tcp_syn(packet) -> ParsedPacket | None:
    ...
    Only processes packets with TCP layer and SYN flag set (no ACK).
    Extracts: TTL, window size, MSS, TCP options for p0f-style fingerprinting.
    """
    try:
...
def parse_dhcpv4(packet) -> ParsedPacket | None:
    ...
    msg_type = None
    requested_addr = None
    client_id = None  # Option 61 — often contains real hardware MAC
    raw_options = {}

    for opt in dhcp.options:
        if isinstance(opt, tuple) and len(opt) >= 2:
            name, value = opt[0], opt[1]
            # Collect all options for anomaly analysis
            raw_options[name] = value
            # Extract named fields for fingerprinting
            if name == "param_req_list":
                ...
                # Type 1 (Ethernet): 01 + 6-byte MAC address
                # NetworkManager sends the real hardware MAC here even when
                # the layer-2 MAC is randomized.
                if isinstance(value, bytes) and len(value) == 7 and value[0] == 1:
                    # Type 1 = Ethernet hardware address
                    client_id = value[1:].hex(":")
                elif isinstance(value, bytes) and len(value) == 6:
                    # Some clients send raw MAC without type prefix
                    client_id = value.hex(":")

    client_mac = bootp.chaddr[:6].hex(":")  # First 6 bytes = MAC

    # Determine the client's actual IP address.
    # DHCP clients send from 0.0.0.0 before they have an IP, so the IP layer
    # source is unreliable.  Prefer: yiaddr (server-assigned) > ciaddr (client
    # current) > requested_addr option > IP layer source.
    client_ip = "0.0.0.0"
    ...
    return ParsedPacket(
        ...
        data={
            ...
            "client_id": client_id,  # Real MAC from Option 61 (may differ from src_mac)
...
def parse_dhcpv6(packet) -> ParsedPacket | None:
    ...
    # Check if this is a DHCPv6 packet
    ...
    # Extract ORO (Option Request Option)
    oro = None
...
def parse_mdns(packet) -> ParsedPacket | None:
        ...
        for i in range(qd_count):
            try:
                ...
                if "._tcp." in qname or "._udp." in qname:
                    # Extract service type (e.g., _airplay._tcp)
    ...
    if dns.an:
        ...
        for i in range(an_count):
            try:
                ...
                # Extract service name from PTR/SRV records
                if hasattr(rr, 'rdata'):
                    if hasattr(rr, 'type') and rr.type == 16:
                        # TXT record (type 16) — parse key=value pairs
                        rdata = rr.rdata
                        if isinstance(rdata, (bytes, bytearray)):
                            # Parse TXT record: series of length-prefixed strings
                            pos = 0
                            while pos < len(rdata):
                                length = rdata[pos]
                                pos += 1
    ...
    # Clean mDNS instance names: strip service suffix and trailing hex hashes
    # "Google-Home-Mini-d9d314d5032b4ae6._googlecast._tcp.local" → "Google-Home-Mini"
    clean_name = name
    if clean_name:
        import re
        if "._tcp." in clean_name or "._udp." in clean_name:
            clean_name = clean_name.split("._")[0]
        clean_name = re.sub(r'-[0-9a-f]{12,}$', '', clean_name, flags=re.IGNORECASE)
    ...
    # Add TXT record data if present
    if txt_records:
        data['txt_records'] = txt_records
        # Extract specific high-value fields from TXT records
        if 'md' in txt_records:
            data['model'] = txt_records['md']  # Google Cast model name
        if 'fn' in txt_records:
            data['friendly_name'] = txt_records['fn']  # User-set device name
        if 'am' in txt_records:
            data['apple_model'] = txt_records['am']  # Apple model ID (e.g., "AudioAccessory5,1")
...
def parse_arp(packet) -> ParsedPacket | None:
    ...
    # Only ARP replies (op=2) or requests (op=1)
...
def parse_tls_client_hello(packet) -> ParsedPacket | None:
    ...
    # Only interested in traffic to port 443 (client -> server)
    if tcp.dport != 443:
        return None
...
def parse_dns(packet) -> ParsedPacket | None:
    ...
    # Only process queries (qr=0), ignore responses (qr=1)
    if dns.qr == 1:
        return None
...
# --- Passive IP Discovery (fallback parser) ---
...
def parse_dns_answer(packet) -> list[ParsedPacket]:
    ...
    if DNS not in packet or not packet[DNS].qr:  # qr=0 is query, qr=1 is response
        return []
    if IP not in packet:
        return []
